...
main(int argc, char **argv)
{
	...
	if ((argc > 1) && ((savefile = fopen(argv[1])) != NULL)) {
		...
		if (ferror(savefile)) {
			...
			die("offset id shorter than expected", fromsc, tosc, NULL);
		offset[offsetlen] = '\0';
	...
	for (long i=0; i<contentlen-UUIDLEN-8; i++) {  // 8 = length of "uuid":"
		if (response[i] == '"') {
			if (!strncmp(&response[i], "\"uuid\":\"", 8)) {
				if (offset[0] == '\0') {
					strncpy(offset, &response[i+8], UUIDLEN);
					break;
				} else {
					for (int j=0; j<UUIDLEN; j++)
						if (response[i+8+j] < offset[j])
							break outer;
						else if (response[i+8+j] > offset[j])
							break;
				}
			}
